...
- A v0.2.0 é o release CORRENTE e as linhas abaixo descrevem o estado vivo
- Entregue na v0.2.0: o Chrome lançado pelo próprio produto fala DevTools por `--remote-debugging-pipe` e não abre porta TCP
- Entregue na v0.2.0: o Xvfb privado exige um `MIT-MAGIC-COOKIE-1`, e um Chrome headed nele é fixado em X11
- Entregue na v0.2.0: execuções headed simultâneas não dividem mais um display, e `display_backend` informa o display realmente usado
- Entregue na v0.2.0: um lançamento com falha mata o grupo de processos inteiro do Chrome, e o `--timeout` durante esse encerramento sai com 124
- Não validado ao vivo na v0.2.0: o caminho do pipe no Windows, macOS, KDE e Sway
- Entregue na v0.1.9: `--headless` e `--browser-mode` alcançam todo modo de janela pelo argv, então `browser_mode` não é mais só XDG
- A v0.1.8 entregou a família anti-detecção e fechou os gaps G2, G4, G8, G9, G11 e G13
- A v0.1.9 fechou identidade, tela e FTL, o emulate só de tela via CDP, o plano webdriver do `--no-stealth` e o cartão de release corrente do README
- A v0.1.9 também nomeou o recorte vivo do fingerprint e acrescentou os gates chrome-mac e eval-nav
...
- A v0.1.8 acrescentou `browser_mode`, padrão `auto`, alcançável só por XDG naquele release
- Desde a v0.1.9 `--headless` e `--browser-mode` alcançam todo modo de janela pelo argv, então aquela leitura só XDG é histórico
...
- GAP-021 parcial: a confiança do parser lighthouse vem de fixtures unit (minimal + chrome-captured LHR)
- O e2e mock do GAP-021 permanece SKIP, então nunca alegar PASS completo do parser lighthouse em e2e
- GAP-022 dups residuais: ~53 duplicatas multi-versão de dependência medidas, poda barata esgotada, residual aceito
- GAP-023 / GAP-024: flags/comandos wishlist do PRD permanecem divergências intencionais — não paridade PRD completa
- Encode AVIF: removido de `grab` (webp permanece), documentado como residual breaking intencional da 0.1.6
...
- Fronteira (não é débito 0.2.0): serviço remoto de change, JA4 no motor HTTP (exige TLS C), ordem de headers no motor HTTP (`HeaderMap` do reqwest não itera por inserção), solvers de desafio
- Reconhecimento de texto em imagem dentro do processo não é fronteira, e foi removido de propósito
- Ele não volta, porque toda LLM que chama já tem visão
...
- No produto agora: path→path `image` / `video` (magic, download SSRF, convert/remux, to-mp3, trim, thumbnail, manifest)
- No produto agora: ffmpeg/ffprobe opcional do SO via XDG `ffmpeg_path` (sem libav linkado)
- No produto agora: `video manifest` resume a estrutura de HLS `.m3u8` e DASH `.mpd` sem baixar mídia
- Fora do produto (honestidade): playback adaptativo HLS/DASH, downloaders yt-dlp/site, encode pure-Rust de produção, batch multi-file JoinSet
- Agentes usam ferramentas externas para esses casos, e nenhum deles pode ser alegado como entregue
