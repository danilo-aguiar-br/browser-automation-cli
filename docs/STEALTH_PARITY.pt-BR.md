...
- As citações de arquivo e linha e os comandos de `Como verificar` foram reauditados em 2026-09-13 contra o binário `0.2.0`
- Essa reauditoria não lançou navegador e não releu nenhum README de referência
...
## O que a 0.2.0 mudou para esta matriz
- O binário `0.2.0` trocou a porta TCP do DevTools do Chrome lançado pela própria CLI por um pipe, como o `CHANGELOG.md` registra em `Security`
- `src/native/cdp/chrome/spawn.rs:171` chama `use_pipe_transport`, que remove todo switch `--remote-debugging-port=` e `--remote-debugging-address=` antes de acrescentar os switches do pipe
- A ponte ainda abre um listener TCP em loopback em `src/native/cdp/pipe/bridge.rs:43`, num path montado a partir de um UUID v4 em `src/native/cdp/pipe/bridge.rs:50`
- A ponte responde `403` a qualquer outro path em `src/native/cdp/pipe/bridge.rs:339`
- A ponte descarta esse listener assim que um cliente completa o handshake em `src/native/cdp/pipe/bridge.rs:283`
- Nenhuma linha mudou de estado por causa do pipe, porque nenhuma linha nomeia a porta do DevTools como vetor de detecção
- Nenhuma sondagem desse listener em loopback feita pela página foi medida antes ou depois da `0.2.0`, então este documento não faz afirmação de detecção sobre o pipe
- O motor Lightpanda e o caminho legado de launch do `chromiumoxide` mantêm o transporte anterior, como a mesma entrada do `CHANGELOG.md` registra
- As correções headed da `0.2.0` fixam `--ozone-platform=x11` em `src/native/cdp/chrome/args.rs:510` somente quando o display privado subiu, e nenhuma linha cobre o display
- A reauditoria de 2026-09-13 moveu 6 citações que tinham deslizado dentro dos próprios arquivos
- Ela também corrigiu 2 células de evidência que o código contradizia e 3 comandos de `Como verificar` cujo `config get` devolve `null` para chave não gravada
...
| GPU relatada igual à GPU que renderiza | zendriver-rs, opt-in `gpu_backend` para renderizar WebGL e WebGPU na GPU real do host em vez do fallback por software | AUSENTE | nenhuma; o patch relata um par de hardware, nenhuma medição deste documento o comparou com a GPU que desenha, e o bundle SwiftShader de `src/native/cdp/chrome/args.rs:350` é inalcançável em produção porque `src/native/cdp/chrome/options.rs:88` define `webgpu` como `false` por padrão e só os testes o ligam | `rg -n "swiftshader" src/native/cdp/chrome/args.rs` e cronometrar um draw pesado de WebGL contra a string de renderer que a página lê |
...
| Defesa | Referência que cobre | Estado na CLI | Evidência na CLI | Como verificar |
| --- | --- | --- | --- | --- |
| Fingerprint TLS JA3 e JA4 | wreq, declarado como controle fino de extensões TLS em vez de strings de fingerprint, com mais de 100 perfis de dispositivo em `wreq-util` | AUSENTE | nenhuma; `Cargo.toml:299` constrói `reqwest` sobre `rustls` com `webpki-roots`, o que emite um ClientHello de rustls | `rg -n "rustls" Cargo.toml` e depois mandar `--engine http` num endpoint que ecoa JA3, comparando com um Chrome real |
...
| Pausa longa entre palavras | pausas aleatórias de pensamento do guise | COBERTO | `src/native/interaction/kinematics/mod.rs:262` implementa `maybe_long_pause` sob o gate `input_word_pause_permille` | `browser-automation-cli --json --filter-rows 'key=input_word_pause_permille' config list-keys`, ler `default` e depois digitar uma frase procurando o intervalo destoante |
...
- `GPU relatada igual à GPU que renderiza` exige medir qual GPU desenha antes que algum relato possa bater com ela, porque o bundle SwiftShader que `src/native/cdp/chrome/args.rs:338` documenta como quatro flags não é alcançado em produção
