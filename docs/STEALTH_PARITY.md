...
- The file and line citations and the `How to verify` commands were re-audited on 2026-09-13 against binary `0.2.0`
- That re-audit launched no browser and re-read no reference README
...
## What 0.2.0 changed for this matrix
- Binary `0.2.0` replaced the DevTools TCP port of a self-spawned Chrome with a pipe, as `CHANGELOG.md` records under `Security`
- `src/native/cdp/chrome/spawn.rs:171` calls `use_pipe_transport`, which removes every `--remote-debugging-port=` and `--remote-debugging-address=` switch before adding the pipe switches
- The bridge still binds a loopback TCP listener at `src/native/cdp/pipe/bridge.rs:43`, on a path built from a v4 UUID at `src/native/cdp/pipe/bridge.rs:50`
- The bridge answers any other path with `403` at `src/native/cdp/pipe/bridge.rs:339`
- The bridge drops that listener once one client completes the handshake at `src/native/cdp/pipe/bridge.rs:283`
- No row changed state because of the pipe, since no row names the DevTools port as a detection vector
- No page-side probe of that loopback listener was measured before or after `0.2.0`, so this document makes no detection claim about the pipe
- The Lightpanda engine and the legacy `chromiumoxide` launch path keep their previous transport, as the same `CHANGELOG.md` entry records
- The headed fixes of `0.2.0` pin `--ozone-platform=x11` at `src/native/cdp/chrome/args.rs:510` only when the private display started, and no row covers the display
- The 2026-09-13 re-audit moved 6 citations that had drifted inside their files
- It also corrected 2 evidence cells that the code contradicted and 3 `How to verify` commands whose `config get` returns `null` for an unset key
...
| Reported GPU matches the GPU actually rendering | zendriver-rs, `gpu_backend` opt-in to render WebGL and WebGPU on the host's real GPU instead of the software fallback | ABSENT | none; the patch reports a hardware pair, no measurement in this document compared it with the GPU that draws, and the SwiftShader bundle at `src/native/cdp/chrome/args.rs:350` is unreachable in production because `src/native/cdp/chrome/options.rs:88` defaults `webgpu` to `false` and only tests set it | `rg -n "swiftshader" src/native/cdp/chrome/args.rs` and time a heavy WebGL draw against the renderer string the page reads |
...
| Init script injected before the HTML parse | patchright, Init Script Shenanigans: Playwright Routes inject JavaScript into HTML requests so `Runtime.enable` is never needed | ABSENT | none; `src/native/stealth/mod.rs:19` states the patches ride `Page.addScriptToEvaluateOnNewDocument`, and `Fetch.enable` is sent only at `src/native/state/collect.rs:153` to serve replacement content and at `src/native/network/domain_filter.rs:223` to filter domains | `rg -n "addScriptToEvaluateOnNewDocument" src/native/stealth/mod.rs` and confirm no `Fetch.fulfillRequest` rewrites a document body with the stealth payload |
...
| Defence | Reference that covers it | State in the CLI | Evidence in the CLI | How to verify |
| --- | --- | --- | --- | --- |
| JA3 and JA4 TLS fingerprint | wreq, stated as fine-grained control over TLS extensions rather than fingerprint strings, with 100+ device profiles in `wreq-util` | ABSENT | none; `Cargo.toml:299` builds `reqwest` on `rustls` with `webpki-roots`, which emits a rustls ClientHello | `rg -n "rustls" Cargo.toml` then send `--engine http` at a JA3 echo endpoint and compare with a real Chrome |
...
| Long pause between words | guise random thinking pauses | COVERED | `src/native/interaction/kinematics/mod.rs:262` implements `maybe_long_pause` gated by `input_word_pause_permille` | `browser-automation-cli --json --filter-rows 'key=input_word_pause_permille' config list-keys`, read `default`, then type a sentence and look for the outlier interval |
...
- `Reported GPU matches the rendering GPU` needs a measurement of which GPU draws before any report can match it, because the SwiftShader bundle that `src/native/cdp/chrome/args.rs:338` documents as four flags is not reached in production
