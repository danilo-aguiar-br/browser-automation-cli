...
fn run_script(lines: &[String]) -> Option<serde_json::Value> {
    ...
    let dir = std::env::temp_dir().join(format!("dialog-gate-{}-{n}", std::process::id()));
...
fn cannot_run() -> bool {
    if binary().is_none() {
        eprintln!(
            "SKIP dialog_if_present_gate: target/debug/browser-automation-cli absent. \
    ...
    if fixture_url().is_none() {
        eprintln!(
            "SKIP dialog_if_present_gate: fixture \
    ...
    if !chrome_ok {
        eprintln!(
            "SKIP dialog_if_present_gate: doctor reports the host is not ready for Chrome. \
             This is NOT a pass."
        );
        return true;
    }
