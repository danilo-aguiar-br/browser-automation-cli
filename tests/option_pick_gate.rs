...
fn run_script(lines: &[String]) -> Option<serde_json::Value> {
    ...
    let dir = std::env::temp_dir().join(format!("option-pick-gate-{}-{n}", std::process::id()));
...
fn cannot_run() -> bool {
    if binary().is_none() {
        eprintln!(
            "SKIP option_pick_gate: target/debug/browser-automation-cli absent. \
    ...
    if fixture_url().is_none() {
        eprintln!(
            "SKIP option_pick_gate: fixture scripts/fixtures/option_pick/page.html absent. \
             This is NOT a pass."
        );
        return true;
    }
    ...
    if !chrome_ok {
        eprintln!(
            "SKIP option_pick_gate: doctor reports the host is not ready for Chrome. \
             This is NOT a pass."
        );
        return true;
    }
