...
impl BrowserProcess {
    ...
    pub fn wait_or_kill(&mut self, _timeout: std::time::Duration) {
        match self {
            BrowserProcess::Lightpanda(p) => p.kill(),
        }
    }
...
impl BrowserManager {
    ...
    pub async fn launch(options: LaunchOptions, engine: Option<&str>) -> Result<Self, String> {
        ...
        let manager = if engine == "lightpanda" {
            ...
            let launched = super::cdp::oxide::launch_with_oxide(&options).await?;
            let ws_url = launched.ws_url.clone();
            let temp_user_data_dir = launched.temp_user_data_dir.clone();
            let client =
                Arc::new(CdpClient::from_browser(launched.browser, launched.handler).await?);
    ...
    pub async fn close(&mut self) -> Result<(), String> {
        ...
        if self.owns_oxide_browser {
            ...
            let res = super::cdp::oxide::finalize_browser(self.client.browser()).await;
    ...
    pub fn resolve_tab_ref(&self, tab_ref: &TabRef) -> Result<u32, String> {
        match tab_ref {
            ...
            TabRef::Label(name) => self
                ...
                .ok_or_else(|| {
                    format!(
                        "No tab with label `{}`; run `browser-automation-cli tab` to list open tabs",
                        name
                    )
                }),
        }
