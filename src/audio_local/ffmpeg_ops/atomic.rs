...
//! Atomic partial output helpers and ffmpeg error mapping for audio.
    ...
    if let Some(parent) = output.parent() {
        if !parent.as_os_str().is_empty() {
            std::fs::create_dir_all(parent)
                .map_err(|e| crate::audio_local::magic::io_path_err(parent, "mkdir", &e))?;
        }
    }
    Ok(())
}
...
pub(super) fn finalize_partial(partial: &Path, output: &Path) -> Result<(), CliError> {
    ...
    std::fs::rename(partial, output).map_err(|e| {
        let _ = std::fs::remove_file(partial);
        crate::audio_local::magic::io_path_err(output, "rename", &e)
    })
}
...
pub(super) fn compact_ffmpeg_stderr(stderr: &[u8]) -> String {
    const MAX: usize = 280;
